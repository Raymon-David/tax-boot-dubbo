package com.foryou.tax.invoiceconsumer.config.shiro.session;

import com.foryou.tax.invoiceapi.pojo.shiro.sesssion.SessionStatus;
import com.foryou.tax.invoiceapi.pojo.user.UserOnlinePojo;
import com.foryou.tax.invoiceapi.pojo.user.UserPojo;
import com.foryou.tax.invoiceapi.service.shiro.session.ShiroSessionRepository;
...
public class CustomSessionManager {
	 ...
	 * @return
	 */
	public  List<UserOnlinePojo> getAllUser() {
		//获取所有session
		Collection<Session> sessions = customShiroSessionDAO.getActiveSessions();
		List<UserOnlinePojo> list = new ArrayList<UserOnlinePojo>();
		
		for (Session session : sessions) {
			UserOnlinePojo bo = getSessionBo(session);
			if(null != bo){
				list.add(bo);
			}
		}
	 ...
	 * @return
	 */
	public UserOnlinePojo getSession(String sessionId) {
		Session session = shiroSessionRepository.getSession(sessionId);
		UserOnlinePojo bo = getSessionBo(session);
		return bo;
	}
	private UserOnlinePojo getSessionBo(Session session){
		//获取session登录信息。
		Object obj = session.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
		if(null == obj){
			return null;
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			if(null != obj && obj instanceof UserPojo){
				//存储session + user 综合信息
				UserOnlinePojo userBo = new UserOnlinePojo((UserPojo)obj);
				//最后一次和系统交互的时间
	...
	public void forbidUserById(Long id, Long status) {
		//获取所有在线用户
		for(UserOnlinePojo bo : getAllUser()){
